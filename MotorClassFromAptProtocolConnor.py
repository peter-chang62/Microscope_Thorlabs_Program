...
class AptMotor(apt.KDC101_PRM1Z8):
    def __init__(self, *args, **kwargs):
        ...
        self.ENC_CNT_MM = 34304.0
        self.VEL_SCALING_FACTOR = 65536
        self.SAMPLING_INTERVAL = 2048 / 6e6
        self.ENC_CNT_MM_S = (
            self.ENC_CNT_MM * self.SAMPLING_INTERVAL * self.VEL_SCALING_FACTOR
        )
        self.ENC_CNT_MM_S2 = (
            self.ENC_CNT_MM * self.SAMPLING_INTERVAL**2 * self.VEL_SCALING_FACTOR
        )
    ...
    def trigger_on(self, flag):
        ...
        # If turning trigger_on to false and trigger_on was originally true,
        # turn off the trigger for the K-cube. If the user is turning the
        # trigger on, this will be caught whenever the position is changed.
    ...
    def set_position(self, value_mm):
        ...
        if self.trigger_on:
            ...
            if pos < value_mm:  # going forward
                self.set_trigger_pos_params(
                    pos, value_mm, self.step_mm, self.pulse_width_ms, 13
                )
            elif pos > value_mm:  # going backwards
                self.set_trigger_pos_params(
                    value_mm, pos, self.step_mm, self.pulse_width_ms, 14
                )
    ...
    def get_velocity_params(self):
        ...
        write_buffer = struct.pack("<6B", 0x14, 0x04, 0x01, 0x00, self.dst, self.src)
        self.write(write_buffer)
    ...
    def set_max_vel(self, max_vel_mm_s):
        ...
        accel_enc_s2 = int(
            round(accel * self.ENC_CNT_MM_S2)
        )  # multiply by ENC_CNT_MM_S2
        max_vel_enc_s = int(
            round(max_vel_mm_s * self.ENC_CNT_MM_S)
        )  # multiply by ENC_CNT_MM_S
        ...
        write_buffer = struct.pack(
            ...
            accel_enc_s2,
            max_vel_enc_s,
        )  # passed in as min_vel, accel, max_vel
        self.write(write_buffer)

    @_auto_connect
    def get_trigger_io_params(self):
        # MGMSG_MOT_REQ_KCUBETRIGCONFIG 0x0524
        write_buffer = struct.pack("<6B", 0x24, 0x05, 0x01, 0x00, self.dst, self.src)
        self.write(write_buffer)

        # MGMSG_MOT_GET_KCUBETRIGCONFIG 0x0525

        # OUTPUT Trigger Modes 10: General purpose logic output (set using the
        # MOD_SET_DIGOUTPUTS message).
        #
        # 11: Trigger output active (level) when motor 'in motion'. The output
        # trigger goes high (5V) or low (0V) (as set in the Trig1Polarity and
        # Trig2Polarity parameters) when the stage is in motion
        #
        # 12: Trigger output active (level) when motor at 'max velocity'.
        #
        # 13: Trigger output active (pulsed) at pre-defined positions moving
        # forward (set using StartPosFwd, IntervalFwd, NumPulsesFwd and
        # PulseWidth parameters in the SetKCubePosTrigParams message). Only one
        # Trigger port at a time can be set to this mode.
        #
        # 14: Trigger output active (pulsed) at pre-defined positions moving
        # backwards (set using StartPosRev, IntervalRev, NumPulsesRev and
        # PulseWidth parameters in the SetKCubePosTrigParams message). Only one
        # Trigger port at a time can be set to this mode
        #
        # 15: Trigger output active (pulsed) at pre-defined positions moving
        # forwards and backward. Only one Trigger port at a time can be set to
        # this mode.
        read_buffer = self.read(0x25, 0x05, req_buffer=write_buffer)
        result = struct.unpack("<6B5H6H", read_buffer)
        [chan_ident, trig1_mode, trig1_pol, trig2_mode, trig2_pol, *reserved] = result[
            6:
        ]
    ...
    @_auto_connect
    def set_trigger_pos_params(
        self, start_mm, stop_mm, step_mm, width_ms, trigger_mode=15
    ):
...
class KDC101(AptMotor):
    ...
    # ________ redundant functions just for naming convention _________________
